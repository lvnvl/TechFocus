...
public abstract class Engine {
    ...
    public Engine(AppiumDriver<WebElement> driver, Config config) {
        this.driver = driver;
        this.config = config;
    }
                    ...
                    if (actionType.equalsIgnoreCase(Action.SLIDE)) {
                        ...
                        if (actionType.equalsIgnoreCase(pers.traveler.constant.Action.CLICK)) {
                            ...
                            if (locationType.equalsIgnoreCase(Action.ID)) {
                            	Log.logInfo("      -> text:" + driver.findElement(By.id(location)).getText());
                            	Log.logInfo("      -> location:" + driver.findElement(By.id(location)).getLocation().toString());
                                driver.findElement(By.id(location)).click();
                            } else if (locationType.equalsIgnoreCase(Action.NAME)) {
                            	Log.logInfo("      -> text:" + driver.findElement(By.name(location)).getText());
                            	Log.logInfo("      -> location:" + driver.findElement(By.name(location)).getLocation().toString());
                                driver.findElement(By.name(location)).click();
                            } else if (locationType.equalsIgnoreCase(Action.XPATH)) {
                            	Log.logInfo("      -> text:" + driver.findElement(By.xpath(location)).getText());
                            	Log.logInfo("      -> location" + driver.findElement(By.xpath(location)).getLocation().toString());
    ...
    protected void acceptAlert(String tips) throws InterruptedException {
        if (null != tips) {
            ...
            if (waitForText(16, keys)) {
            	Log.logError("error wait for 允许："+driver.findElement(By.xpath(tips.split(">>")[1])).getText()
            			+driver.findElement(By.xpath(tips.split(">>")[1])).getTagName());
                driver.findElement(By.xpath(tips.split(">>")[1])).click();
                Log.logError("允许   clicked!!");
            }
        }
//        TimeUnit.SECONDS.sleep(config.getInterval());
    }
