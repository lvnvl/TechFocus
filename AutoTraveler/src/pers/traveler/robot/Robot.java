    ...
    protected String beforeTravel() {
        ...
        try {
            ...
            if (null != cmd && !cmd.isEmpty()) {
                ...
                StartAppiumServer startAppiumServer = StartAppiumServer.getStartAppiumServer();
                startAppiumServer.setCommand(cmd);
                startAppiumServer.setPort(port);
...
//        System.out.println("driver init done!");
    ...
    public void travel() {
        try {
            ...
            //关闭appium监测线程
            StartAppiumServer.getStartAppiumServer().setCheck(false);
...
//                PicToAvi.convertPicToAvi(pngDir, suffix, aviFileName, fps, mWidth, mHeight);
                ...
                CmdUtil.run(CmdUtil.isWindows()?CmdConfig.WIN_KILL_APPIUM_SERVER:CmdConfig.LINUX_KILL_APPIUM_SERVER);
                
                /*
                 * 调用process_result.sh脚本对遍历结果进行输出
                 * */
                CmdUtil.run(CmdConfig.RUN_POSTTEST_STEP + pngDir + " " + "output" +
                 File.separator + date + File.separator + time + config.getUdid() + File.separator + "logs");  
            } catch (InterruptedException e) {
                e.printStackTrace();
            }catch (Exception e) {
                e.printStackTrace();
