...
public class ImagesToMovie implements ControllerListener, DataSinkListener
{
    ...
    public void saveMovie(MediaLocator outML, String descriptor, VideoFormat formatDesired, String fileToSave) throws IOException, NoProcessorException, NotRealizedError, NoDataSinkException
    {        
...
//        if ((dsink = createDataSink(p, outML)) == null)
        System.out.println("fileToSave is " + fileToSave);
        if ((dsink = createDataSink(p, MovieUtils.createMediaLocator(fileToSave))) == null)
        {
        	Log.logError("创建数据池DataSink失败，ImagesToMovie,122");
    ...
    DataSink createDataSink(Processor p, MediaLocator outML) throws NotRealizedError, NoDataSinkException, IOException
    {
        ...
        try{
        	...
        	Log.logError("ImagesToMovie exception line 187 throws :" + e.getMessage());
        }
        dsink.open();
