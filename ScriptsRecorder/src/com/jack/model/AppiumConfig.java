...
public class AppiumConfig {
	...
	private StringFilterImpl stringFilter = new StringFilterImpl();
...
//		System.out.println("uri raw path:"+u.getRawPath());
		...
		mPackage = stringFilter.grep(appInfo, "package:").split("'")[1].trim();
		mActivity = stringFilter.grep(appInfo, "launchable-").split("'")[1].trim();
		port = p;
//		System.out.println(mPackage+"===");System.out.println(mActivity);
	...
	public void close(){
		String portInfo = CmdUtil.run(
				CmdConfig.CHECK_APPIUM_SERVER_WIN.replaceAll("#port#", String.valueOf(port)));
		if(portInfo != null && portInfo.length() > 0){
			// get pid of the process
			String[] info = portInfo.split("\\s+");
			String pid = info[info.length - 1];
			...
			try {
				...
				new Thread(new Runnable(){
					...
					public void run() {
						...
						CmdUtil.run(CmdConfig.START_APPIUM_SERVER_WIN.replaceAll("#port#", String.valueOf(port)));
					}
