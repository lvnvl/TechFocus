...
//    String LINUX_KILL_APPIUM_SERVER = "ps -A|grep node|grep -v grep|awk 'NR=1 {print $1}'|xargs kill -9";
    ...
    String KILL_APPIUM_SERVER_LINUX = "netstat -ap |grep #port#|grep node|awk '{print $NF}'|awk -F '/' '{print $1}'|xargs kill -9";
    
    // check appium server
    String CHECK_APPIUM_SERVER_LINUX = "netstat -ano|grep #port#";
    String CHECK_APPIUM_SERVER_WIN = "netstat -ano|findstr #port#|findstr LISTENING";
    
    // start appium server
    String START_APPIUM_SERVER_WIN = "appium -p #port# --session-override --command-timeout 7200";
}